...
C언어 및 TCP/IP 소켓 API를 기반으로 구현된 콘솔 환경의 설문조사 및 투표 시스템에 대한 기술 명세서입니다. 본 시스템의 핵심 아키텍처는 다중 클라이언트의 동시 접속 및 상호작용을 지원하기 위한 멀티스레딩이며, 데이터 관리의 안정성과 사용자 중심의 기능 제공을 핵심 목표로 설정하였습니다.
